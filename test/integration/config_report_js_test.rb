...
class ConfigReportJSTest < IntegrationTestWithJavascript
  ...
  test "reports for host" do
    ...
    has_selector?(".foreman-search-bar .pf-v6-c-text-input-group__text-input", text: "host = #{report.host.name}", wait: 3)
    assert_equal "host = #{report.host.name}", find('.foreman-search-bar .pf-v6-c-text-input-group__text-input').value
  end
end
