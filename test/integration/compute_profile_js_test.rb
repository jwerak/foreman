...
class ComputeProfileJSTest < IntegrationTestWithJavascript
  ...
  test "edit compute attribute page" do
    ...
    assert page.has_selector?('.pf-v6-c-page__main-breadcrumb .active', :text => compute_profiles(:one).name), "#{compute_profiles(:one).name} was expected in the breadcrumb active, but was not found"
